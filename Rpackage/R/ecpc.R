...
ecpc <- function(Y,X,groupings,groupings.grouplvl=NaN,
                 hypershrinkage,unpen=NULL,intrcpt=T,model,postselection=F,maxsel=30,
                 lambda="ML",fold=10,sigmasq=NaN,w=NaN,
                 nsplits=100,weights=T,
                 Y2=NaN,X2=NaN,compare=T,
                 mu=F,normalise=F,nIt=1,betaold=NaN){
  #-1. Description input --------------------------------------------------------------------------
  #
  #Data and co-data:
  ...
  if(any(hypershrinkage == "hierTree")){
    #nIt<-1
  ...
  while(Itr<=nIt){
    ...
    }else    print(paste("Compute group penalty estimates"))
        ...
        if(is.nan(tausq)){
          if(is.nan(lambdashat[2])){
            ...
            switch(hypershrinkage,
                   ...
                   "lasso"={#TD: adapt lasso&hiertree, check other hyperpenalties on inclusion function taus
                     meanWhalf <- mean(diag(Wminhalf)^-1)
            ...
            profPlot <- plot(log10(lambdas),FRSS) #profile plot lambda vs RSS
...
### Other functions 
#function to select covariates a posteriori----
...
#function to estimate maximum marginal likelihood estimates for linear model----
...
#function to multiply slices of 3D-arrays----
...
simDat <- function(n,p,n2=20,muGrp,varGrp,indT,sigma=1,model='linear',flag=F){
  ...
  X <- mvrnorm(n,rep(0,p),diag(rep(1,p))) #X~MVN(0,1)
  X2 <- mvrnorm(n2,rep(0,p),diag(rep(1,p))) #X2 independent data
...
#do CV (TD: update)
exactcv <- function(hyperlambdas,Y,X,ind,model,intrcpt,lambda=1,sigmasq=NaN,tausq=NaN,mu=NaN,folds,T1=NaN,T2=NaN){
  #Given lambda_1,lambda_2, compute MSE on left out fold 
  #Return average over all folds
  MSE <- rep(NaN,length(folds))
  for(i in 1:length(folds)){
    #fit model on fold left out for specific extra penalties
    data <- GRridgemeans(Y[-folds[[i]]],X[-folds[[i]],],ind=apply(ind,2,which.max),Y[folds[[i]]],X[folds[[i]],],model=model,intrcpt=intrcpt,
                         lambda=lambda,sigmasq=sigmasq,mu=mu,tausq=tausq,hyperlambdas=hyperlambdas,T1=T1,T2=T2,
                         pen2=F,innercv=F) 
    MSE[i] <- data$MSEecpc
  }
  return(mean(MSE))
...
#function to discretise continuous co-data hierarchically----
#Output: list of groups of covariates of varying size
splitMedian <- function(values,index,depth,minGroupSize=50,first=T,split="left"){
  #split: "both" or "left" if both groups have to be split recursively or only left, lower-valued group
  if(missing(depth)){
  ...
  if(depth>1){ #TD: improve recursion for leaves only splitting in one side
    if(split=="both"){
      ...
      return(c(groups,
               splitMedian(values[indSmall],index[indSmall],depth-1,first=F,split=split)))
    }else{
      stop("split should be one of both or left")
    }
  }else{
...
#function to obtain grouping on group level for hierarhical groups----
...
obtainHierarchy <- function(grouping,penalty="LOG"){
  if(penalty=="LOG"){
    ...
    groupings.grplvl <- lapply(grouping,function(i){as.numeric(which(sapply(grouping,function(j){all(i%in%j)})))})
  }else if(penalty=="GL"){
    ...
    groupings.grplvl <- lapply(grouping,function(j){as.numeric(which(sapply(grouping,function(i){all(i%in%j)})))})
  }
  return(groupings.grplvl)
}

#function to fit hierarchical lasso using LOG penalty----
...
#Function to compute inverse gamma penalty parameters with mode 1 for given group size and hyperpenalty lambda----
...
visualiseGroupingweights <- function(dfGrps,GroupingNames,hist=F,boxplot=T,jitter=T,ps=1.5,width=0.5){
  ...
  if(hist){
    p1 <- ggplot(dfGrps)+
      ...
      labs(x="Prior variance grouping weight")+
      theme_bw()+
    ...
    p1<-ggplot(SummdfGrps)+
      ...
      labs(y="Prior variance grouping weight",x="Grouping")+
      theme_bw()+
      theme(axis.text.x=element_text(size=12),
            axis.text.y=element_text(size=12),
            axis.title.x=element_text(size=14),
